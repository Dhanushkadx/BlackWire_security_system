...
//AT+CIURC
 sendCheckReply(F("AT+CIURC=0"), ok_reply);
  delay(100);
  flushInput();
...
uint8_t Adafruit_FONA :: waitCallResp(uint8_t timeout){
	...
	//uint8_t ret = 0;
	 ...
	 if (IsStringReceived(PSTR("MO RING")))
	 {
		 return 1;
	 } 
	 else if (IsStringReceived(PSTR("MO CONNECTED")))
	 {
		 return 2;
	 }
	 else if (IsStringReceived(PSTR("BUSY")))
	 {
		  return 3;
	 }
	
	 else if (IsStringReceived(PSTR("NO CARRIER")))
	 {
		  return 4;
	 }
	 else if (IsStringReceived(PSTR("NO ANSWER")))
	 {
		 return 4;
	 }
	 else if (IsStringReceived(PSTR("NO DIALTONE")))
	 {
		  return 5;
	 }
...
uint8_t Adafruit_FONA :: getSMSIndex(uint8_t required_status){
	 uint8_t ret_val = 0;
	 char *p_char;
	 byte status;
	 switch (required_status) {
		 case SMS_UNREAD:
		 getReply(F("AT+CMGL=\"REC UNREAD\""));
		
		
		 break;
		 case SMS_READ:
		 
		 getReply(F("AT+CMGL=\"REC READ\""));
		
		 break;
		 case SMS_ALL:
		
		 getReply(F("AT+CMGL=\"ALL\""));
	...
	//DEBUG_PRINTLN(replybuffer);
	 ...
	 else if(strncmp("OK",replybuffer,2)==0){
		  ret_val = 0;}
...
**********************************************************/
uint8_t Adafruit_FONA::IsStringReceived(char const *compare_string)
{
     char *ch;
     uint8_t ret_val = 0;
     ...
     if(replybuffer) {
          ...
          */
          ...
          ch = strstr(replybuffer, compare_string);
          if (ch != NULL) {
               ret_val = 1;
               /*#ifdef DEBUG_PRINT
...
bool Adafruit_FONA::deleteSMSV2(uint8_t message_index) {
	 ...
	 mySerial->println((int)message_index);
	 readline(500, true);
...
bool Adafruit_FONA :: deleteAllSMS(){
	 if (!sendCheckReply(F("AT+CMGF=1"), ok_reply))
	 return false;
	 
	  if (!sendCheckReply(F("AT+CMGDA=\"DEL ALL\""), ok_reply))
	  return false;
	/*//mySerial->println("AT+CMGD=1,4");
	mySerial->println("AT+CMGD=\"DEL READ\"");*/
	return true;
}
...
**********************************************************/
int8_t Adafruit_FONA::GetSMSx(uint8_t position, char *phone_number,byte max_phone_len, char *SMS_text, uint16_t max_SMS_len,uint16_t *readlen)
{
     uint8_t ret_val = -1;
     char *p_char;
     char *p_char1;
	...
	/*  char cmd[20];
	  memset(cmd,'\0',20);
	  sprintf(cmd,"AT+CMGR=%d",position);*/
     // getReply(F("AT+CMGR="), message_index, 1000);  //  do not print debug!
     mySerial->print(F("AT+CMGR="));
     mySerial->println(position);
     readline(1000); // timeout
	 DEBUG_PRINT(F("Reply1: ")); DEBUG_PRINTLN(replybuffer);
	  // parse it out...
	  if (IsStringReceived("OK"))
	  {
		  ret_val = GETSMS_NO_SMS; 
	  } 
	  else if (IsStringReceived("ERROE"))
	  {
		  ret_val = GETSMS_NO_SMS;
	  }
	  else if (IsStringReceived("+CMGR"))
	  {
		   ...
		   // ---------------------------
			...
			char replybuffer_copy[255];
		    memcpy (replybuffer_copy, replybuffer, strlen(replybuffer)+1);
		   p_char = strchr(replybuffer_copy,',');
			 ...
			 if (!parseReply(F("+CMGR:"), &thesmslen, ',', 11)) {
				 *readlen = 0;
				 ret_val = GETSMS_NO_SMS; 
				  ...
				  readRaw(thesmslen);
			//DEBUG_PRINT(F("Reply2: ")); DEBUG_PRINTLN(replybuffer);
				  flushInput();

				  uint16_t thelen = min(max_SMS_len, (uint16_t)strlen(replybuffer));
				  strncpy(SMS_text, replybuffer, thelen);
				  SMS_text[thelen] = 0; // end the string
