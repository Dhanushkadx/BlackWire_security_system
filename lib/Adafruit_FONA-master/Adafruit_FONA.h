...
#define FONA_DEFAULT_TIMEOUT_MS 3000
...
public:
  ...
  bool deleteSMSV2(uint8_t message_index); 
  bool deleteAllSMS();
  ...
  uint8_t waitCallResp(uint8_t timeout);
  bool hangUp(void);
  bool pickUp(void);
  bool callerIdNotification(bool enable, uint8_t interrupt = 0);
  bool incomingCallNumber(char *phonenum);
  int8_t waitForDTMF(char *tone,uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
